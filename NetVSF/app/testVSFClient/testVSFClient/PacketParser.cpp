...
int PacketParser::Parse2(unsigned char* pBuffer, int iLen)
{
	...
	if (bHasHeader)
	{
		...
		{
			m_iTmpBufLen = 0;
			sprintf(szMsg, "VSFClient PacketParser::Parse() - lose Tailer!![ulLength = %d , ulPacketLen = %d , m_iTmpBufLen = %d]\n", ulLength, ulPacketLen, m_iTmpBufLen);
			OutputDebugStringA(szMsg);
		}
	}
...
int PacketParser::CheckPacketLen(unsigned char* pBuffer, int iLen)
{
	...
	else
	{
		...
		sprintf(szMsg, "VSFClient PacketParser::Parse() - no Header!! [iRet = %d]\n", iRet);
		OutputDebugStringA(szMsg);
