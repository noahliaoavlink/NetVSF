...
void libJpeg::Test()
{
	...
	sprintf(szInFileName,"%s\\20170527061543_29.bmp", szAPPath);

	img_file.Load(szInFileName);
	img_file.GetWidth();
	img_file.GetHeight();
	img_file.GetBuffer();
	...
	sprintf(szOutFileName, "%s\\test_libjpeg.jpg", szAPPath);
	...
	if (iDeRet == 1)
	{
		sprintf(szOutFileName, "%s\\test_libjpeg.bmp", szAPPath);
		img_file.SaveBmpFile(szOutFileName, pDeBuf, iOutW, iOutH);
	}
