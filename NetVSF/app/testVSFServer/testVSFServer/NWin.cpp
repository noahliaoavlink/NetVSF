...
void CNWin::SetCallback(WinMsgCallback2* pCallback)
{
...
LRESULT CNWin::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	switch(message)
	{
		...
		case WM_WINDOWPOSCHANGED:
			{
				if(m_pCallback)
					m_pCallback->WinMsg(m_hWnd,message,(void*)wParam,(void*)lParam);
			}
			break;
		case WM_SYSCOMMAND:
			{
				if(m_pCallback)
					return (long)m_pCallback->WinMsg(m_hWnd,message,(void*)wParam,(void*)lParam);
			}
			break;
			/*
		case WM_TIMER:
